...
namespace TexMap {
	...
	public:
		...
		void BackgroundSubtraction(bool use_keyframe = true);

		void ConstructVertree();

		void SetPropVec(vector<vector<float2>> propVec, int layer);

		void SetPropVec_key(vector<vector<float2>> propVec, int layer);

		void BackPropVec_key();

		void SetValidFrame(bool use_keyframe);
		...
		void RecomputeWeight_normal();

		void InterpKeytoAll();

		void SetBoundary(vector<vector<vector<int>>> boundLabels, vector<vector<int>> resultLabels, vector<int> all2valid);

		void SetBoundary_fine(vector<vector<vector<int>>> boundLabels, vector<vector<int>> resultLabels, vector<int> all2valid);

		void SetLabelResults(vector<vector<int>> resultLabels);

		void SpatiotemporalSampleAndCalpos(string streamPath, string NR_MeshPathAndPrefix, int startIdx, int endIdx);

		void Calpos(string streamPath, string NR_MeshPathAndPrefix, int startIdx, int endIdx);
		...
		void Get_VT_layer_key(vector<hostVertex>& hVs, vector<hostTriangle>& hTs, int layer);

		void Get_V_layer(vector<hostVertex> &hVs, int layer);
		
		void Push_VT_layer(vector<hostVertex> &hVs, vector<hostTriangle> &hTs, int layer);

		void ForceSetnow2ori();

		void Push_VT_layer_key(vector<hostVertex>& hVs, vector<hostTriangle>& hTs, int layer);

		void Push_V_layer(vector<hostVertex> &hVs, int layer);
		
		bool SaveResult(string FileName);

		bool LoadResult(string FileName);
		...
		vector<uint> keyFrame_idx;
		vector<uint> validFrame_table;

		vector<vector<bool>> Bound_vec;
		vector<vector<bool>> Label_vec;
	...
	private:
		...
		vector<vector<hostVertex>> layer_Vertices_key;
		vector<vector<hostTriangle>> layer_Triangles_key;
