...
GLuint loadStream_custom(const char* imagepath, vector<cv::Mat> &image_data, int startIdx, int endIdx) {

	cout << "Start extracting atlas video" << endl;
	char frameNameBuffer[512] = { 0 };
	unsigned int width, height;
	sprintf(frameNameBuffer, "%s/Frame_%06d.png", imagepath, startIdx);
	if(exists_test0(string(imagepath) + "/Smooth_" + zeroPadding(startIdx, 6) + ".png"))
		sprintf(frameNameBuffer, "%s/Smooth_%06d.png", imagepath, startIdx);
	cv::Mat image = cv::imread(frameNameBuffer, CV_LOAD_IMAGE_UNCHANGED);
	width = image.cols;
	height = image.rows;
	GLuint textureID;
	glGenTextures(1, &textureID);

	image_data.resize(endIdx - startIdx);
	image_data.reserve(endIdx - startIdx);
	for (int frameIdx = startIdx; frameIdx < endIdx; frameIdx++) {
		printProgBar(((float)frameIdx / (endIdx - 1.0)) * 100);
		sprintf(frameNameBuffer, "%s/Frame_%06d.png", imagepath, frameIdx);
		if (exists_test0(string(imagepath) + "/Smooth_" + zeroPadding(frameIdx, 6) + ".png"))
			sprintf(frameNameBuffer, "%s/Smooth_%06d.png", imagepath, frameIdx);
		image_data[frameIdx] = cv::imread((string)frameNameBuffer);
		//cv::cvtColor(image_data[frameIdx], image_data[frameIdx], CV_BGRA2BGR);
	}
	printProgBar(100);
	cout << "\n End extracting atlas video" << endl;

	// "Bind" the newly created texture : all future texture functions will modify this texture
	glBindTexture(GL_TEXTURE_2D, textureID);

	// Give the image to OpenGL
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_BGR, GL_UNSIGNED_BYTE, image_data[0].data);
