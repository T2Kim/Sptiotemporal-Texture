...
private:
	...
	GLuint time_stamp = 0;
	GLuint end_stamp = 0;
	GLuint atlas_stamp = 0;
	TexMap::Optimizer *optimizer_ptr;
	uint cW, cH;
	vector<cv::Mat> images;

	bool is_video = false;
	vector<cv::Mat> atlas_data;
	vector<cv::Mat> atlas_data2;
	vector<cv::Mat> atlas_data3;

	bool is_atlas2 = false;
	bool is_atlas3 = false;
...
public:
	...
	glm::vec3 upload_UVAtlas_video(string ScanMeshPathAndPrefix, string atlasPath, string tex_filename, int startIdx, int endIdx) {
		is_video = true;
		...
		if (mesh_extension == "obj") {
			...
			if (!OpenMesh::IO::read_mesh(om_mesh, filename.c_str(), opt))
			{
				std::cerr << "read error\n";
				exit(1);
			}
			...
			unsigned int numV = om_mesh.n_vertices();
			unsigned int numP = om_mesh.n_faces();
			unsigned int numE = 0;
			vertices_offset = tex_vec.size();

			for (MyMesh::VertexIter v_it = om_mesh.vertices_begin(); v_it != om_mesh.vertices_end(); ++v_it) {
				glm::vec4 v;
				v.x = om_mesh.point(*v_it)[0];
				v.y = om_mesh.point(*v_it)[1];
				v.z = om_mesh.point(*v_it)[2];
				v.w = 1.0;
				tmp_vert_vec.push_back(v);
			}
			for (MyMesh::FaceIter f_it = om_mesh.faces_begin(); f_it != om_mesh.faces_end(); ++f_it) {
				for (auto fv_it = om_mesh.fv_iter(*f_it); fv_it.is_valid(); ++fv_it) {
					GLuint e;
					e = fv_it->idx();
					tmp_elements.push_back(e);
				}
		...
		if (tex_vec.size() > 0) {
			...
			glActiveTexture(GL_TEXTURE0);
			glEnable(GL_TEXTURE_2D);
			text = loadStream_custom((atlasPath + "/video/").c_str(), atlas_data, startIdx, endIdx);
			const char* uniform_name;
			uniform_name = "tex";
			TextureID = glGetUniformLocation(program, uniform_name);
			glDisable(GL_TEXTURE_2D);
			...
			glActiveTexture(GL_TEXTURE1);
			glEnable(GL_TEXTURE_2D);
			text_s = loadPNG_edgesmooth((atlasPath + "/multi.png").c_str(), tex_vec, elements);
			const char* uniform_name_s;
			uniform_name_s = "tex_s";
			TextureID_s = glGetUniformLocation(program, uniform_name_s);
			glDisable(GL_TEXTURE_2D);
	...
	glm::vec3 upload_UVAtlas_video2(string ScanMeshPathAndPrefix, string atlasPath, string atlasPath2, string tex_filename, int startIdx, int endIdx) {
		is_video = true;
		is_atlas2 = true;
		...
		if (tex_vec.size() > 0) {
			...
			glActiveTexture(GL_TEXTURE1);
			glEnable(GL_TEXTURE_2D);
			text_s = loadStream_custom((atlasPath2 + "/video/").c_str(), atlas_data2, startIdx, endIdx);
			const char* uniform_name_s;
			uniform_name_s = "tex_s";
			TextureID_s = glGetUniformLocation(program, uniform_name_s);
			glDisable(GL_TEXTURE_2D);
			...
			glActiveTexture(GL_TEXTURE2);
			glEnable(GL_TEXTURE_2D);
			text_n = loadPNG_edgesmooth((atlasPath + "/multi.png").c_str(), tex_vec, elements);
			const char* uniform_name_n;
			uniform_name_n = "tex_n";
			TextureID_n = glGetUniformLocation(program, uniform_name_n);
			glDisable(GL_TEXTURE_2D);
	...
	glm::vec3 upload_UVAtlas_video3(string ScanMeshPathAndPrefix, string atlasPath, string atlasPath2, string atlasPath3, string tex_filename, int startIdx, int endIdx) {
		is_video = true;
		is_atlas2 = true;
		is_atlas3 = true;
		...
		if (tex_vec.size() > 0) {
			...
			glActiveTexture(GL_TEXTURE2);
			glEnable(GL_TEXTURE_2D);
			text_n = loadStream_custom((atlasPath3 + "/video/").c_str(), atlas_data3, startIdx, endIdx);
			const char* uniform_name_n;
			uniform_name_n = "tex_n";
			TextureID_n = glGetUniformLocation(program, uniform_name_n);
			glDisable(GL_TEXTURE_2D);
	...
	void draw() {
		...
		if (is_video && atlas_stamp != time_stamp) {
			//glDeleteTextures(1, &text);
			//glGenTextures(1, &text);

			//glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, cW, cH, 0, GL_BGR, GL_UNSIGNED_BYTE, atlas_data[time_stamp].data);
			//glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, cW, cH, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
			glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, ATLAS_SIDE, ATLAS_SIDE, GL_BGR, GL_UNSIGNED_BYTE, atlas_data[time_stamp].data);
		}
		...
		if (is_video && atlas_stamp != time_stamp && is_atlas2) {
			glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, ATLAS_SIDE, ATLAS_SIDE, GL_BGR, GL_UNSIGNED_BYTE, atlas_data2[time_stamp].data);
		}
		...
		glActiveTexture(GL_TEXTURE2);
		glEnable(GL_TEXTURE_2D);
		if (is_video && atlas_stamp != time_stamp && is_atlas3) {
			glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, ATLAS_SIDE, ATLAS_SIDE, GL_BGR, GL_UNSIGNED_BYTE, atlas_data3[time_stamp].data);
		}
		/*glTexGeni(GL_S, GL_TEXTURE_GEN_MODE, GL_SPHERE_MAP);
		glTexGeni(GL_T, GL_TEXTURE_GEN_MODE, GL_SPHERE_MAP);*/
		...
		if (is_video && atlas_stamp != time_stamp) {
			...
			cout << flag  << "-> frame: " << time_stamp << endl;
			atlas_stamp = time_stamp;
		}
		/*glActiveTexture(GL_TEXTURE1);
		glTexGeni(GL_S, GL_TEXTURE_GEN_MODE, GL_SPHERE_MAP);
		glTexGeni(GL_T, GL_TEXTURE_GEN_MODE, GL_SPHERE_MAP);
		...
		//glUniformMatrix4fv(uniform_m, 1, GL_FALSE, glm::value_ptr(cv2gl * this->object2world));
		//glUniformMatrix4fv(uniform_m, 1, GL_FALSE, glm::value_ptr(this->object2world * cv2gl));
		/* Apply flags */
...
namespace TexMap {
	...
	public:
		...
		int init_resources_video(string ScanMeshPathAndPrefix, string tex_filename, string bmp_filename, const char * vshader_filename, const char * fshader_filename, int startIdx, int endIdx);
		int init_resources_video2(string ScanMeshPathAndPrefix, string tex_filename, string bmp_filename, string bmp_filename2, const char* vshader_filename, const char* fshader_filename, int startIdx, int endIdx);
		int init_resources_video3(string ScanMeshPathAndPrefix, string tex_filename, string bmp_filename, string bmp_filename2, string bmp_filename3, const char* vshader_filename, const char* fshader_filename, int startIdx, int endIdx);
		...
		int init_resources_UVAtlas(string tex_filename);
		int init_resources_UVAtlas2(string tex_filename);
		int init_resources_UVAtlas_sub(string tex_filename);
		int init_resources_UVAtlas_video(const char * vshader_filename, const char * fshader_filename, string tex_filename, int startIdx, int endIdx);
		...
		void GenAtlas_UVAtlas_float(const char* vshader_filename, const char* fshader_filename, string tex_filename, uchar* result, bool draw, int width = ATLAS_SIDE, int height = ATLAS_SIDE);
		...
		void GenAtlas_UVAtlas_video(const char* vshader_filename, const char* fshader_filename, string tex_filename, uchar* result, int targetIdx, bool draw = false, int width = ATLAS_SIDE, int height = ATLAS_SIDE);
		void GenAtlas_UVAtlas_video_mask(const char* vshader_filename, const char* fshader_filename, string tex_filename, uchar* result, int targetIdx, bool draw = false, int width = ATLAS_SIDE, int height = ATLAS_SIDE);
		void GenAtlas_UVAtlas_texel_video(const char* vshader_filename, const char* fshader_filename, string tex_filename, uchar* result, int targetIdx, bool draw = false, int width = ATLAS_SIDE, int height = ATLAS_SIDE);
		void GenAtlas_UVAtlas_mask_video(const char* vshader_filename, const char* fshader_filename, string tex_filename, uchar* result, int targetIdx, bool draw = false, int width = ATLAS_SIDE, int height = ATLAS_SIDE);
