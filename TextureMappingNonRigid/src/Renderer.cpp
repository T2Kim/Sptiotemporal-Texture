...
void Renderer::onKeyboard(unsigned char key, int x, int y) {
	...
	case 'p':
		...
		for (int i = 0; i < end_idx; i++) {
			...
			main_object4D.set_time_stamp(i);
			if (true) {
				string filename = data_root_path + "/unit_test/" + unit_test_path + "/" + flag + "/Frame" + zeroPadding(main_object4D.get_time_stamp(), 3) + ".png";
				draw();
...
int Renderer::init_resources_video(string ScanMeshPathAndPrefix, string tex_filename, string bmp_filename, const char* vshader_filename, const char* fshader_filename, int startIdx, int endIdx) {
	...
	objmov = main_object4D.upload_UVAtlas_video(ScanMeshPathAndPrefix, bmp_filename, tex_filename, startIdx, endIdx);
...
int Renderer::init_resources_UVAtlas(string tex_filename) {
	if (tex_filename != "")
...
int Renderer::init_resources_UVAtlas_video(const char* vshader_filename, const char* fshader_filename, string tex_filename, int startIdx, int endIdx) {
	...
	for (int i = startIdx; i < endIdx; i++) {
		GenAtlas_UVAtlas_video(v_path_atlas.c_str(), f_path_atlas.c_str(), tex_filename, data, i, true);
		GenAtlas_UVAtlas_video_mask(v_path_atlas.c_str(), f_path_atlas.c_str(), tex_filename, data, i, true);
	}
	delete[] data;
	return 1;
}
...
void Renderer::logic() {
	...
	if (object4D_play && fps_frames % ((int)(fps_now / fps_play) + 1) == 0 || object4D_play) {
		string flag;
		string filename;
		if (main_object4D._flags[1]) {
			if(main_object4D._flags[0])
	...
	if (0) {
		float g_nearPlane = 0.1;
		float g_farPlane = 10.0;
		world2camera[0][0] = D_C_EXT[0];
		world2camera[1][0] = D_C_EXT[1];
		world2camera[2][0] = D_C_EXT[2];
		world2camera[3][0] = D_C_EXT[3];

		world2camera[0][1] = D_C_EXT[4];
		world2camera[1][1] = D_C_EXT[5];
		world2camera[2][1] = D_C_EXT[6];
		world2camera[3][1] = D_C_EXT[7];

		world2camera[0][2] = D_C_EXT[8];
		world2camera[1][2] = D_C_EXT[9];
		world2camera[2][2] = D_C_EXT[10];
		world2camera[3][2] = D_C_EXT[11];

		/*world2camera[0][0] = D_C_EXT[0];
		world2camera[1][0] = D_C_EXT[4];
		world2camera[2][0] = D_C_EXT[8];
		world2camera[3][0] = D_C_EXT[3];

		world2camera[0][1] = D_C_EXT[1];
		world2camera[1][1] = D_C_EXT[5];
		world2camera[2][1] = D_C_EXT[9];
		world2camera[3][1] = D_C_EXT[7];

		world2camera[0][2] = D_C_EXT[2];
		world2camera[1][2] = D_C_EXT[6];
		world2camera[2][2] = D_C_EXT[10];
		world2camera[3][2] = D_C_EXT[11];*/
		...
		glm::mat4 cv2gl = glm::mat4(1);
		cv2gl[1][1] = -1;
		cv2gl[2][2] = -1;

		//world2camera = world2camera* cv2gl;
		world2camera = cv2gl * world2camera;

		//world2camera = glm::inverse(world2camera);

		//world2camera = glm::mat4(1);

		camera2screen[0][0] = 2 * COFLX / COIMX;
		camera2screen[1][0] = 0;
		camera2screen[2][0] = 1 - 2 * COCPX / COIMX;
		camera2screen[3][0] = 0;

		camera2screen[0][1] = 0;
		camera2screen[1][1] = 2 * COFLY / COIMY;
		camera2screen[2][1] = 2 * COCPY / COIMY - 1;
		camera2screen[3][1] = 0;

		camera2screen[0][2] = 0;
		camera2screen[1][2] = 0;
		camera2screen[2][2] = (g_farPlane + g_nearPlane) / (g_nearPlane - g_farPlane);
		camera2screen[3][2] = (2 * g_farPlane * g_nearPlane) / (g_nearPlane - g_farPlane);
...
void imwrite_thread(cv::Mat img, string imgName) {
...
void Renderer::GenAtlas_UVAtlas(const char* vshader_filename, const char* fshader_filename, string tex_filename, uchar* result, bool draw, int width, int height) {
	...
	for (uint i = 0; i < nImage; i++) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, width_img, height_img, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
		/*cv::Mat3f ch3Image;
		ch3Image.create(ATLAS_SIDE, ATLAS_SIDE);
		optimizer->mapper4D_ptr->colorImages[optimizer->mapper4D_ptr->validFrame_table[i]].convertTo(ch3Image, CV_32FC3);
		ch3Image /= 255.0;
		glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width_img, height_img, GL_RGB, GL_FLOAT, ch3Image.data);*/
		glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width_img, height_img, GL_RGB, GL_UNSIGNED_BYTE, optimizer->mapper4D_ptr->colorImages[optimizer->mapper4D_ptr->validFrame_table[i]].data);
		/*cv::imshow("111", optimizer->mapper4D_ptr->colorImages[optimizer->mapper4D_ptr->keyFrame_idx[i]]);
		cv::waitKey(0);*/
	...
	if (draw) {
		vector<float> buf(4 * width * height);
		glPixelStorei(GL_PACK_ALIGNMENT, 1);
		glReadPixels(0, 0, width, height, GL_RGBA, GL_FLOAT, buf.data());
		cv::Mat4f ch4Image;
		cv::Mat4b ch4Image_uc4;
		ch4Image.create(ATLAS_SIDE, ATLAS_SIDE);
		ch4Image_uc4.create(ATLAS_SIDE, ATLAS_SIDE);
		memcpy(ch4Image.data, buf.data(), sizeof(float) * ATLAS_SIDE * ATLAS_SIDE * 4);
		ch4Image *= 255.0;
		ch4Image.convertTo(ch4Image_uc4, CV_8UC4);
		cv::imwrite(data_root_path + "/atlas/" + atlas_path + "/" + optimizer->opt_mode + ".png", ch4Image_uc4);
		cv::resize(ch4Image_uc4, ch4Image_uc4, cv::Size(800, 800));
		cv::imshow("Atlas", ch4Image_uc4);
		cv::waitKey(10);
		buf.clear();
	}
...
void Renderer::GenAtlas_UVAtlas_texel(const char* vshader_filename, const char* fshader_filename, string tex_filename, uchar* result, bool draw, int width, int height) {
	...
	for (uint i = 0; i < nImage; i++) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width_img, height_img, 0, GL_RGB, GL_UNSIGNED_BYTE, optimizer->mapper4D_ptr->colorImages[optimizer->mapper4D_ptr->validFrame_table[i]].data);
		...
		memcpy(ori_img.data, optimizer->mapper4D_ptr->colorImages[i].data, sizeof(uchar)*height_img* width_img * 3);
...
void Renderer::GenAtlas_UVAtlas_video(const char* vshader_filename, const char* fshader_filename, string tex_filename, uchar* result, int targetIdx, bool draw, int width, int height) {
	...
	optimizer->PrepareRend_UVAtlas_video(targetIdx);
	...
	for (uint i = 0; i < nImage; i++) {
		...
		glTexBuffer(GL_TEXTURE_BUFFER, GL_RGB, tbo);*/
		//glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width_img, height_img, GL_RGB, GL_UNSIGNED_BYTE, &optimizer->hCImages[i*width_img*height_img * 3]);
		glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width_img, height_img, GL_RGB, GL_UNSIGNED_BYTE, optimizer->mapper4D_ptr->colorImages[i].data);
		//glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width_img, height_img, GL_RGB, GL_UNSIGNED_BYTE, &tmpImg[i*width_img*height_img * 3]);
	...
	if (draw) {
		...
		ch3Image.create(ATLAS_SIDE, ATLAS_SIDE);
		memcpy(ch3Image.data, buf.data(), sizeof(uchar) * ATLAS_SIDE * ATLAS_SIDE * 3);
		cv::imwrite(data_root_path + "/atlas/" + atlas_path + "/video/Frame_" + zeroPadding(targetIdx, 6)+ ".png", ch3Image);
		cv::resize(ch3Image, ch3Image, cv::Size(800, 800));
		cv::imshow("Atlas", ch3Image);
		cv::waitKey(10);
	}
...
void Renderer::GenAtlas_UVAtlas_video_mask(const char* vshader_filename, const char* fshader_filename, string tex_filename, uchar* result, int targetIdx, bool draw, int width, int height) {
	...
	optimizer->PrepareRend_UVAtlas_video_mask(targetIdx);
	...
	for (uint i = 0; i < nImage; i++) {
		...
		optimizer->GetAtlasInfoi_UVAtlas_mask(&atlas_coord, &img_coord, &tri_idx, i);
		for (int ti = 0; ti < tri_idx.size(); ti++) {
			atlas_coord[ti * 6] = main_object_tex.tex_coords[main_object_tex.tex_elements[tri_idx[ti] * 3]].x;
			atlas_coord[ti * 6 + 1] = main_object_tex.tex_coords[main_object_tex.tex_elements[tri_idx[ti] * 3]].y;
			atlas_coord[ti * 6 + 2] = main_object_tex.tex_coords[main_object_tex.tex_elements[tri_idx[ti] * 3 + 1]].x;
	...
	if (draw) {
		...
		ch3Image.create(ATLAS_SIDE, ATLAS_SIDE);
		memcpy(ch3Image.data, buf.data(), sizeof(uchar) * ATLAS_SIDE * ATLAS_SIDE * 3);
		cv::imwrite(data_root_path + "/atlas/" + atlas_path + "/video/Mask_" + zeroPadding(targetIdx, 6) + ".png", ch3Image);
	}
